...
THook(bool, "?useLegacyKnockback@KnockbackRules@@YA_NAEBVLevel@@@Z", void* knockbackRules, void* level) { return true; }
...
THook(void, "?knockback@ServerPlayer@@UEAAXPEAVActor@@HMMMMM@Z",
    ...
    int lastHurtCause = direct_access<int>(player, 0x700);
    if (lastHurtCause == (int)ActorDamageCause::None) return;
    ...
    if (lastHurtCause == (int)ActorDamageCause::Projectile) {
    ...
    else if (lastHurtCause == (int)ActorDamageCause::EntityAttack) {
    ...
    float knockbackResistanceValue = knockbackResistanceAttribute->currentVal;
    if (knockbackResistanceValue > 0.0f) {
        power *= 1.0f - knockbackResistanceValue;
        height *= 1.0f - knockbackResistanceValue;
    }
    ...
    if (f > 0.0f) {
        ...
        if (posDelta.y > heightCap) { // heightcap behavior behaves weirdly on bedrock
    ...
    if (currentTick - attackTimestamp[player] <= 1) {
        posDelta.x *= settings.knockbackReductionFactor; // attacking on the same tick that you receive knockback will set lower values (aka "reducing")
        posDelta.z *= settings.knockbackReductionFactor;
    }
